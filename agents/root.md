...
<rule>If working on the Go backend service (`app/`), you MUST read `<path>agents/backend.md</path>` before taking action.</rule>
<rule>If working on the Next.js frontend (`fe-agent/`), you MUST read `<path>agents/frontend.md</path>` before taking action.</rule>
</routing_rules>

<global_constraints>
<constraint>Always `cd` into the appropriate directory (`app/` or `fe-agent/`) before running build tools or scripts.</constraint>
