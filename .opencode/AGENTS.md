...
*   If working on the Go backend service (`app/`), you MUST read `.opencode/backend.md` before taking action.
*   If working on the Next.js frontend (`fe-agent/`), you MUST read `.opencode/frontend.md` before taking action.
...
Custom skills specific to this repository are stored in `.opencode/skills/`. 

When working on a specialized task:
1. Check `.opencode/skills/` for existing custom workflows.
2. If you need to use one, load it via the `skill` tool using its name (the directory name in `.opencode/skills/`).
